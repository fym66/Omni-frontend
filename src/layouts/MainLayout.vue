  ...
  <div class="app-frame">
    <header class="topbar">
      <RouterLink class="app-logo" to="/" aria-label="Omni 首页">
        <span>O</span>
        <strong>Omni 精选</strong>
      </RouterLink>

      <form class="search-box" @submit.prevent="submitSearch">
        <input v-model="query" type="search" placeholder="搜索你感兴趣的内容" />
        <button type="submit" :disabled="!searchable">搜索</button>
      </form>

      <nav class="top-actions" aria-label="主要导航">
        <RouterLink class="upload-link" to="/upload">投稿</RouterLink>
    ...
    <aside class="sidebar" aria-label="主导航">
      <RouterLink v-for="item in navItems" :key="`${item.label}-${item.to}`" :to="item.to">
        <span>{{ item.icon }}</span>
        {{ item.label }}
      </RouterLink>
    </aside>
